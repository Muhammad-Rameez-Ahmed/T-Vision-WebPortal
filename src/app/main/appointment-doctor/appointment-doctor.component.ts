...
import { Appointments, Status } from './appointment-doctor.model';
...
export class AppointmentDoctorComponent implements OnInit {
    ...
    show() {
        ...
        if (this.selectedStatus === 3) {
            this.getAllHistory(this.selectedStatus, this.selectedDate)
        }
        else {
    ...
    openDetails(item: Appointments) {
        ...
        this.router.navigate(['/dashboard/doc-appointments/details'])
    }
